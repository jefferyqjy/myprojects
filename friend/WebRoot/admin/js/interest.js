function initializeEvents() {
	$('.datatable').dataTable({
		...
		"bServerSide" : true,
		"sAjaxSource" : "/friend/admin/interest/list.spring",
	    "sServerMethod" : "POST" ,
	    "bProcessing" : false,
	    "bPaginate": true,
		"bLengthChange" : true,
...
function createAction(id) {
	var inhtml = '<a class="btn btn-success" href="/friend/admin/interest/view.spring?id=' + id + '">';
	inhtml += '<i class="icon-zoom-in icon-white"></i>View</a> ';
	inhtml += '<a class="btn btn-info" href="/friend/admin/interest/preUpdate.spring?id=' + id + '">';
	inhtml += '<i class="icon-edit icon-white"></i>Edit</a> ';
	inhtml += '<a class="btn btn-danger" href="/friend/admin/interest/delete.spring?id=' + id + '">';
