function initializeEvents() {
	$('.datatable').dataTable({
		...
		"bServerSide" : true,
		"sAjaxSource" : "/friend/admin/subject/list.spring",
	    "sServerMethod" : "POST" ,
	    "bProcessing" : false,
	    "bPaginate": true,
		"bLengthChange" : true,
...
function makeCollumnDef() {
	return [
	        ...
	        { "fnRender" : function (oObj, sVal) {
	      	        return oObj.aData.universityName;
	           },
	           "bVisible" : true ,
	           "aTargets" : [ 2 ]
	        },
...
function makeCollomns(){
	return [{ "mDataProp" : "id", "sHeight":"15px"}, 
        	{ "mDataProp" : "name"},
        	{ "mDataProp" : "universityName"},
        	{ "mDataProp" : "checked"}];
}
function createAction(id) {
	var inhtml = '<a class="btn btn-success" href="/friend/admin/subject/view.spring?id=' + id + '">';
	inhtml += '<i class="icon-zoom-in icon-white"></i>View</a> ';
	inhtml += '<a class="btn btn-info" href="/friend/admin/subject/preUpdate.spring?id=' + id + '">';
	inhtml += '<i class="icon-edit icon-white"></i>Edit</a> ';
	inhtml += '<a class="btn btn-danger" href="/friend/admin/subject/delete.spring?id=' + id + '">';
