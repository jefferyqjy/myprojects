...
-- 导出  表 friend.com_pro_black_member 结构
CREATE TABLE IF NOT EXISTS `com_pro_black_member` (
...
-- 正在导出表  friend.com_pro_black_member 的数据：~0 rows (大约)
/*!40000 ALTER TABLE `com_pro_black_member` DISABLE KEYS */;
/*!40000 ALTER TABLE `com_pro_black_member` ENABLE KEYS */;


-- 导出  表 friend.com_pro_boke 结构
...
-- 正在导出表  friend.com_pro_boke 的数据：~4 rows (大约)
...
-- 导出  表 friend.com_pro_boke_message 结构
...
-- 正在导出表  friend.com_pro_boke_message 的数据：~7 rows (大约)
...
-- 导出  表 friend.com_pro_city 结构
CREATE TABLE IF NOT EXISTS `com_pro_city` (
  `ID` int(11) NOT NULL AUTO_INCREMENT,
  `NAME` varchar(50) COLLATE utf8_bin NOT NULL DEFAULT '' COMMENT '名称',
  `PINYIN` varchar(50) COLLATE utf8_bin NOT NULL DEFAULT '' COMMENT '拼音',
  `PROVINCE` int(11) NOT NULL DEFAULT '0' COMMENT '省份Id',
  PRIMARY KEY (`ID`)
) ENGINE=InnoDB AUTO_INCREMENT=7 DEFAULT CHARSET=utf8 COLLATE=utf8_bin COMMENT='城市表';

-- 正在导出表  friend.com_pro_city 的数据：~6 rows (大约)
/*!40000 ALTER TABLE `com_pro_city` DISABLE KEYS */;
INSERT INTO `com_pro_city` (`ID`, `NAME`, `PINYIN`, `PROVINCE`) VALUES
	(1, '南京', 'NANJIGN', 1),
	(2, '南通', 'NANTONG', 1),
	(3, '北京市', 'BEIJING', 4),
	(4, '济南市', 'JINAN', 2),
...
-- 导出  表 friend.com_pro_friend_group 结构
...
-- 正在导出表  friend.com_pro_friend_group 的数据：~10 rows (大约)
...
INSERT INTO `com_pro_friend_group` (`ID`, `NAME`, `USER_ID`, `CRE_DATE`) VALUES
	...
	(13, '我的好友', 7, '2016-04-14 22:08:52');
/*!40000 ALTER TABLE `com_pro_friend_group` ENABLE KEYS */;


-- 导出  表 friend.com_pro_friend_group_user 结构
...
-- 正在导出表  friend.com_pro_friend_group_user 的数据：~9 rows (大约)
...
-- 导出  表 friend.com_pro_interest 结构
...
-- 正在导出表  friend.com_pro_interest 的数据：~3 rows (大约)
...
-- 正在导出表  friend.com_pro_member 的数据：~7 rows (大约)
...
INSERT INTO `com_pro_member` (`ID`, `USERNAME`, `USERPASSWORD`, `GENDER`, `AGE`, `ADDRESS`, `UNIVERSITY`, `STATUS`, `PROVINCE`, `CITY`, `SUBJECT`, `YEAR`, `STUNO`) VALUES
	...
	(7, 'jeffery', '111111', NULL, NULL, NULL, 4, 0, 2, 5, 1, 2009, '0913042018');
/*!40000 ALTER TABLE `com_pro_member` ENABLE KEYS */;


-- 导出  表 friend.com_pro_member_interest 结构
...
-- 正在导出表  friend.com_pro_member_interest 的数据：~6 rows (大约)
...
-- 导出  表 friend.com_pro_message 结构
...
-- 正在导出表  friend.com_pro_message 的数据：~9 rows (大约)
...
INSERT INTO `com_pro_message` (`ID`, `FROM_USER_ID`, `TO_USER_ID`, `MESSAGE`, `CRE_DATE`, `STATUS`, `FROM_USER_NAME`) VALUES
	...
	(9, 7, 6, 'aa', '2016-04-14 22:35:06', 0, 'jeffery');
/*!40000 ALTER TABLE `com_pro_message` ENABLE KEYS */;


-- 导出  表 friend.com_pro_photo 结构
...
-- 正在导出表  friend.com_pro_photo 的数据：~50 rows (大约)
...
-- 导出  表 friend.com_pro_photobox 结构
...
-- 正在导出表  friend.com_pro_photobox 的数据：~3 rows (大约)
...
-- 导出  表 friend.com_pro_photobox_message 结构
...
-- 正在导出表  friend.com_pro_photobox_message 的数据：~5 rows (大约)
...
-- 导出  表 friend.com_pro_province 结构
CREATE TABLE IF NOT EXISTS `com_pro_province` (
  `ID` int(11) NOT NULL AUTO_INCREMENT,
  `NAME` varchar(50) NOT NULL DEFAULT '' COMMENT '省份名称',
  `PINYIN` varchar(50) NOT NULL DEFAULT '' COMMENT '省份拼音',
  PRIMARY KEY (`ID`)
) ENGINE=InnoDB AUTO_INCREMENT=5 DEFAULT CHARSET=utf8 COMMENT='省份表';

-- 正在导出表  friend.com_pro_province 的数据：~4 rows (大约)
/*!40000 ALTER TABLE `com_pro_province` DISABLE KEYS */;
INSERT INTO `com_pro_province` (`ID`, `NAME`, `PINYIN`) VALUES
	(1, '江苏省', 'JIANGSU'),
	(2, '山东省', 'SHANDONG'),
	(3, '浙江省', 'ZHEJAING'),
	(4, '北京市', 'BEIJING');
/*!40000 ALTER TABLE `com_pro_province` ENABLE KEYS */;


-- 导出  表 friend.com_pro_report 结构
CREATE TABLE IF NOT EXISTS `com_pro_report` (
  `ID` int(11) NOT NULL AUTO_INCREMENT,
  `USERID` int(11) NOT NULL DEFAULT '0' COMMENT '被举报人',
  `REASON` varchar(200) NOT NULL DEFAULT '' COMMENT '举报原因',
  `REPORTER` varchar(50) NOT NULL DEFAULT '' COMMENT '举报人',
  `REPORTTIME` datetime NOT NULL DEFAULT '1900-01-01 00:00:00' COMMENT '举报时间',
  PRIMARY KEY (`ID`)
) ENGINE=InnoDB AUTO_INCREMENT=2 DEFAULT CHARSET=utf8 COMMENT='举报记录表';

-- 正在导出表  friend.com_pro_report 的数据：~1 rows (大约)
/*!40000 ALTER TABLE `com_pro_report` DISABLE KEYS */;
INSERT INTO `com_pro_report` (`ID`, `USERID`, `REASON`, `REPORTER`, `REPORTTIME`) VALUES
	(1, 6, '打广告', '7', '2016-04-15 00:09:06');
/*!40000 ALTER TABLE `com_pro_report` ENABLE KEYS */;


-- 导出  表 friend.com_pro_role 结构
...
-- 正在导出表  friend.com_pro_role 的数据：~2 rows (大约)
...
-- 导出  表 friend.com_pro_subject 结构
CREATE TABLE IF NOT EXISTS `com_pro_subject` (
  `ID` int(11) NOT NULL AUTO_INCREMENT,
  `NAME` varchar(50) NOT NULL DEFAULT '',
  `UNIVERSITY` int(11) NOT NULL DEFAULT '0',
  PRIMARY KEY (`ID`)
) ENGINE=InnoDB AUTO_INCREMENT=5 DEFAULT CHARSET=utf8 COMMENT='大学专业表';

-- 正在导出表  friend.com_pro_subject 的数据：~4 rows (大约)
/*!40000 ALTER TABLE `com_pro_subject` DISABLE KEYS */;
INSERT INTO `com_pro_subject` (`ID`, `NAME`, `UNIVERSITY`) VALUES
	(1, '计服', 4),
	(2, '软件工程', 4),
	(3, '网络工程', 4),
	(4, '计算机师范', 4);
/*!40000 ALTER TABLE `com_pro_subject` ENABLE KEYS */;


-- 导出  表 friend.com_pro_university 结构
...
-- 正在导出表  friend.com_pro_university 的数据：~7 rows (大约)
...
-- 正在导出表  friend.com_pro_user 的数据：~0 rows (大约)
...
-- 导出  表 friend.com_pro_user_role 结构
...
-- 正在导出表  friend.com_pro_user_role 的数据：~0 rows (大约)
