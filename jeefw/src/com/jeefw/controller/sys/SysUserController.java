...
public class SysUserController extends JavaEEFrameworkBaseController<SysUser> implements Constant {
	...
	public void login(SysUser sysUserModel, HttpServletRequest request, HttpServletResponse response) throws IOException {
		...
		session.setAttribute("SESSION_sysUserWithAvatar", sysUserWithAvatar);
		...
		if (!file.isEmpty()) {
			if (file.getSize() > 2097152) {
				...
				try {
					...
					String uploadroot="/static/upload/img/";
					String filedirectory=request.getRealPath(uploadroot)+File.separator+ DateFormatUtils.format(new Date(), "yyyyMM");
					...
					SysUser sysUserWithAvatar = sysUserService.getSysUserWithAvatar(sessionuser);
	...
	public String activity(HttpServletRequest request, HttpServletResponse response) throws IOException {
		return "back/activity/activity";
	}
	@RequestMapping("/applycandidate")
	public String applycandidate(HttpServletRequest request, HttpServletResponse response) throws IOException {
		return "back/activity/applycandidate";
	}
	@RequestMapping("/applyactivity")
	public String applyactivity(HttpServletRequest request, HttpServletResponse response) throws IOException {
		return "back/activity/applyactivity";
	}
