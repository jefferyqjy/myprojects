...
$.app = {
    ...
    initNotification : function() {
        ...
        var initMenu = function(dataList, hasUnread) {
            ...
            var content = "";
            $(dataList).each(function (index, data) {
                content = content + contentTemplate.replace("{unread}", data.readed ? "" : "unread").replace("{title}", data.title).replace("{date}", data.date);
            });
...
//            });
    ...
    initDatetimePicker : function(options) {
        ...
        $('.date:not(.custom)').each(function() {
            ...
            var pickTime = $(this).find("[data-format]").data("format").toLowerCase().indexOf("hh:mm") != -1;
            var pickSeconds = $(this).find("[data-format]").data("format").toLowerCase().indexOf("hh:mm:ss") != -1;
            var config = {pickDate : pickDate,
                         pickTime : pickTime,
            ...
            $date.find(":input").focus(function() {$date.find(".icon-calendar,.icon-time,.icon-date").click();});
            $date.attr("initialized", true);
        });
    },
    initAreaPicker : function(that) {
        //初始化 datetime picker
        $(that).find('.date:not(.custom)').each(function() {
            var $date = $(this);
    ...
    toggleDisplayTable : function(tableEle) {
        var openIcon = "icon-plus-sign";
        var closeIcon = "icon-minus-sign";
        $(tableEle).find("tr .toggle-child").click(function() {
            var $a = $(this);
            ...
            $.app.confirm({
                ...
                ok : function() {
                    ...
                    $.post(options.batchDeleteUrl, ids, function() {
                        $trs.remove();
                        if(!$.table.changeBtnStateCustom){
                        	$.table.changeBtnState(table);
                        }else{
                        	$.table.changeBtnStateCustom(table);
                        }
                    });
...
$.table = {
	...
	changeBtnStateCustom:undefined,
    ...
    initSearchForm : function(table) {
        ...
        searchForm.find(".btn-clear-search").off("click").on("click",function() {
        ...
        var exports = function(table,searchForm){
       	 var url = $.table.tableURL(table);
            url = $.table.removeSearchParam(url, searchForm);
            url = $.table.removePageParam(url, searchForm);
            url+="/export";
        ...
        if(searchForm.is("[data-change-search=true]")) {
            searchForm.find(":input:not(:button,:submit,:reset,.nosearch)").off("change").on("change", function(e) {
        ...
        searchForm.find(".btn-export").off("click").on("click", function() {
        	exports(table,searchForm);
            return false;
        });
    ...
    initTableBtn : function($table, urlPrefix) {
        ...
        if($table.hasClass("table-dblclick-toggle")){
        	$table.children("tbody").children("tr").off("dblclick").on("dblclick", function() {
                $(this).find(".toggle-child").trigger("click");
            });
        }
...
$.movable = {
     ...
     * @param urlPrefix
     */
    initMoveableTable : function(table,setting) {
        if(!table.length) {
            return;
        }
