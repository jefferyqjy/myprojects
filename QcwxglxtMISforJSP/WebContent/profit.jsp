...
<script language="JavaScript">
	function doSearch() {
		if (document.all.searchValue.value == "") {
			...
			window.location.href = "Profit?currentPage=1&&pagerMethod=1&&operator=0&&queryName="
	...
	function doSearchAll() {
		window.location.href = "Profit?operator=1&&currentPage=1&&pagerMethod=1";
	}
</script>
									...
									<td height="30">
										<table width="100%" border="0" cellspacing="0" cellpadding="0">
											<tr align="center" style="font-size: 25px;">
												...
												<td width="1101" background="img/tab_05.gif">利润管理</td>
													...
													<table>
														<tr>
															<td><img src="img/311.gif" width="16" height="16" />
																<span class="STYLE4">结账利润列表</span></td>
														</tr>
													</table>
													<table width="99%" border="0" align="center"
														...
														<tr>
															<td width="10%" height="30" background="img/tab_14.gif"
																bgcolor="#FFFFFF" align="center">利润单编号</td>
															<td width="10%" height="30" background="img/tab_14.gif"
																bgcolor="#FFFFFF" align="center">结账单编号</td>
															<td width="10%" height="30" background="img/tab_14.gif"
																bgcolor="#FFFFFF" align="center">零件费</td>
															<td width="10%" height="30" background="img/tab_14.gif"
																bgcolor="#FFFFFF" align="center">维修费用</td>
															<td width="15%" height="30" background="img/tab_14.gif"
																bgcolor="#FFFFFF" align="center">总消费</td>
															<td width="10%" height="30" background="img/tab_14.gif"
																bgcolor="#FFFFFF" align="center">结账日期</td>
															<td width="10%" height="30" background="img/tab_14.gif"
																bgcolor="#FFFFFF" align="center">零件采购金额</td>
															<td width="10%" height="30" background="img/tab_14.gif"
																bgcolor="#FFFFFF" align="center">利润</td>
														</tr>
														<tbody align="center" onmouseover="changeto()"
															onmouseout="changeback()">
															<c:forEach var="item" items="${profits}" varStatus="wage">
																<tr align="center" bgcolor="#FFFFFF" height="25">
																	<td>${item.pId}</td>
																	<td>${item.cId}</td>
																	<td>${item.checkouts.partprice}</td>
																	<td>${item.checkouts.repaircost}</td>
																	<td>${item.checkouts.xiaofei}</td>
																	<td>${item.checkouts.checkoutsdate}</td>
																	<td>${item.cost}</td>
																	<td>${item.profit}</td>
																</tr>
															</c:forEach>
														...
														<tr>
															<td colspan="9">共${totalRows}行&nbsp;
																第${currentPage}页&nbsp; 共${totalPage}页&nbsp; <c:if
																	test="${operator==0}">
																	<a
																		href="Profit?operator=0&&queryName=${queryName}&&queryValue=${queryValue}&&currentPage=${currentPage}&&pagerMethod=1">首页</a>
																	<a
																		href="Profit?operator=0&&queryName=${queryName}&&queryValue=${queryValue}&&currentPage=${currentPage}&&pagerMethod=2">上一页</a>
																	<a
																		href="Profit?operator=0&&queryName=${queryName}&&queryValue=${queryValue}&&currentPage=${currentPage}&&pagerMethod=3">下一页</a>
																	<a
																		href="Profit?operator=0&&queryName=${queryName}&&queryValue=${queryValue}&&currentPage=${currentPage}&&pagerMethod=4">尾页</a>
																</c:if> <c:if test="${operator==1}">
																	<a
																		href="Profit?operator=1&&currentPage=${currentPage}&&pagerMethod=1">首页</a>
																	<a
																		href="Profit?operator=1&&currentPage=${currentPage}&&pagerMethod=2">上一页</a>
																	<a
																		href="Profit?operator=1&&currentPage=${currentPage}&&pagerMethod=3">下一页</a>
																	<a
																		href="Profit?operator=1&&currentPage=${currentPage}&&pagerMethod=4">尾页</a>
																</c:if>
															</td>
														</tr>
													</table></td>
