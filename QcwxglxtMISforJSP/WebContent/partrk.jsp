...
<script language="JavaScript">
	function doSearch() {
		if (document.all.searchValue.value == "") {
			...
			window.location.href = "Partrk?currentPage=1&&pagerMethod=1&&p_operator=0&&queryName="
	...
	function doSearchAll() {
		window.location.href = "Partrk?p_operator=1&&currentPage=1&&pagerMethod=1";
	}
	function doAdd() {
		window.location.href = "MaintainPartrk?p_operatorStr=add&&operatorStr=0";
	}
</script>
				...
				<table width="1050" cellspacing="0" cellpadding="0">
					<tr>
						<td align="center" valign="top" height="390"><form
								name="Partrk_form" method="post"></form>
									...
									<td height="30">
										<table width="100%" border="0" cellspacing="0" cellpadding="0">
											<tr align="center" style="font-size: 25px;">
												...
												<td width="1101" background="img/tab_05.gif">零件采购入库管理</td>
														...
														<tr align="center" height="50">
															<td valign="top" class="mt10">请选择查询条件:<select
																name="searchName" class="select">
																	<option value="partrkID">采购编号</option>
																	<option value="supplier.supName">供应商全称</option>
																	<option value="Partrk.empName">员工名称</option>
													...
													<table>
														<tr>
															<td><img src="img/311.gif" width="16" height="16" />
																<span class="STYLE4">零件采购入库信息列表</span></td>
														</tr>
													</table>
													<table width="99%" border="0" align="center"
														...
														<tr>
															<td width="8%" height="30" background="img/tab_14.gif"
																bgcolor="#FFFFFF" align="center">采购编号</td>
															...
															<td width="10%" height="30" background="img/tab_14.gif"
																bgcolor="#FFFFFF" align="center">供应商名称</td>
															<td width="7%" height="30" background="img/tab_14.gif"
																bgcolor="#FFFFFF" align="center">总金额</td>
															<td width="10%" height="30" background="img/tab_14.gif"
																bgcolor="#FFFFFF" align="center">采购日期</td>
															<td width="7%" height="30" background="img/tab_14.gif"
																bgcolor="#FFFFFF" align="center">查看采购明细</td>
															<td width="7%" height="30" background="img/tab_14.gif"
																bgcolor="#FFFFFF" align="center">入库退货</td>
														</tr>
														<tbody align="center" onmouseover="changeto()"
															onmouseout="changeback()">
															<c:forEach var="item" items="${partrks}"
																varStatus="partrks">
																<tr align="center" bgcolor="#FFFFFF" height="35">
																	<td>${item.partrkID}</td>
																	<td>${item.empName}</td>
																	<td>${item.supName}</td>
																	<td>${item.partrktotal}</td>
																	<td>${item.rkdate}</td>
																	<td><a
																		href="Partrk?p_operator=3&&partrkID=${item.partrkID}">查看采购明细</a></td>
																	<td><a
																		href="Partrk?p_operator=4&&partrkID=${item.partrkID}">入库退货</a></td>
																</tr>
															</c:forEach>
														</tbody>
													</table>
														...
														<tr>
															<td colspan="9">共${totalRows}行&nbsp;
																第${currentPage}页&nbsp; 共${totalPage}页&nbsp; <c:if
																	test="${p_operator==0}">
																	<a
																		href="Partrk?p_operator=0&&queryName=${queryName}&&queryValue=${queryValue}&&currentPage=${currentPage}&&pagerMethod=1">首页</a>
																	<a
																		href="Partrk?p_operator=0&&queryName=${queryName}&&queryValue=${queryValue}&&currentPage=${currentPage}&&pagerMethod=2">上一页</a>
																	<a
																		href="Partrk?p_operator=0&&queryName=${queryName}&&queryValue=${queryValue}&&currentPage=${currentPage}&&pagerMethod=3">下一页</a>
																	<a
																		href="Partrk?p_operator=0&&queryName=${queryName}&&queryValue=${queryValue}&&currentPage=${currentPage}&&pagerMethod=4">尾页</a>
																</c:if> <c:if test="${p_operator==1}">
																	<a
																		href="Partrk?p_operator=1&&currentPage=${currentPage}&&pagerMethod=1">首页</a>
																	<a
																		href="Partrk?p_operator=1&&currentPage=${currentPage}&&pagerMethod=2">上一页</a>
																	<a
																		href="Partrk?p_operator=1&&currentPage=${currentPage}&&pagerMethod=3">下一页</a>
																	<a
																		href="Partrk?p_operator=1&&currentPage=${currentPage}&&pagerMethod=4">尾页</a>
																</c:if>
															</td>
														</tr>
													</table></td>
