...
<script language="JavaScript">
	function doSearch() {
		if (document.all.searchValue.value == "") {
			...
			window.location.href = "Evaluation?currentPage=1&&pagerMethod=1&&operator=0&&queryName="
	...
	function doSearchAll() {
		window.location.href = "Evaluation?operator=1&&currentPage=1&&pagerMethod=1";
	}
</script>
				...
				<table width="1050" cellspacing="0" cellpadding="0">
					<tr>
						<td align="center" valign="top" height="390"><form
								name="Evaluation_form" method="post"></form>
									...
									<td height="30">
										<table width="100%" border="0" cellspacing="0" cellpadding="0">
											<tr align="center" style="font-size: 25px;">
												...
												<td width="1101" background="img/tab_05.gif">客户评价管理</td>
														...
														<tr align="center" height="50">
															<td valign="top" class="mt10">请选择查询条件:<select
																name="searchName" class="select">
																	<option value="eID">评价表编号</option>
																	<option value="checkouts.cID">结账单编号</option>
													...
													<table>
														<tr>
															<td><img src="img/311.gif" width="16" height="16" />
																<span class="STYLE4">评价信息列表</span></td>
														</tr>
													</table>
													<table width="99%" border="0" align="center"
														...
														<tr>
															<td width="13%" height="30" background="img/tab_14.gif"
																bgcolor="#FFFFFF" align="center">评价表编号</td>
															<td width="13%" height="30" background="img/tab_14.gif"
																bgcolor="#FFFFFF" align="center">结账单编号</td>
															<td width="7%" height="30" background="img/tab_14.gif"
																bgcolor="#FFFFFF" align="center">评价时间</td>
															<td width="7%" height="30" background="img/tab_14.gif"
																bgcolor="#FFFFFF" align="center">评价内容</td>
														...
														<tbody align="center" onmouseover="changeto()"
															...
															<c:forEach var="item" items="${evaluations}" varStatus="evaluation">
																<tr align="center" bgcolor="#FFFFFF" height="25">
																	<td>${item.eID}</td>
												<td>${item.cID}</td>
												<td>${item.evaluatime}</td>
												<td>${item.evaluation}</td>
																	<td><a href="Evaluation?operator=4&&eID=${item.eID}"><input
														...
														<tr>
															<td colspan="9">共${totalRows}行&nbsp;
																第${currentPage}页&nbsp; 共${totalPage}页&nbsp; <c:if
																	test="${operator==0}">
																	<a
																		href="Evaluation?operator=0&&queryName=${queryName}&&queryValue=${queryValue}&&currentPage=${currentPage}&&pagerMethod=1">首页</a>
																	<a
																		href="Evaluation?operator=0&&queryName=${queryName}&&queryValue=${queryValue}&&currentPage=${currentPage}&&pagerMethod=2">上一页</a>
																	<a
																		href="Evaluation?operator=0&&queryName=${queryName}&&queryValue=${queryValue}&&currentPage=${currentPage}&&pagerMethod=3">下一页</a>
																	<a
																		href="Evaluation?operator=0&&queryName=${queryName}&&queryValue=${queryValue}&&currentPage=${currentPage}&&pagerMethod=4">尾页</a>
																</c:if> <c:if test="${operator==1}">
																	<a
																		href="Evaluation?operator=1&&currentPage=${currentPage}&&pagerMethod=1">首页</a>
																	<a
																		href="Evaluation?operator=1&&currentPage=${currentPage}&&pagerMethod=2">上一页</a>
																	<a
																		href="Evaluation?operator=1&&currentPage=${currentPage}&&pagerMethod=3">下一页</a>
																	<a
																		href="Evaluation?operator=1&&currentPage=${currentPage}&&pagerMethod=4">尾页</a>
																</c:if>
															</td>
														</tr>
													</table></td>
