...
<script language="JavaScript">
	function doSearch() {
		if (document.all.searchValue.value == "") {
			...
			window.location.href = "Arrange?currentPage=1&&pagerMethod=1&&operator=0&&queryName="
	...
	function doSearchAll() {
		window.location.href = "Arrange?operator=1&&currentPage=1&&pagerMethod=1";
	}
	function doAdd() {
				...
				<table width="1050" cellspacing="0" cellpadding="0">
					<tr>
						<td align="center" valign="top" height="390"><form
								name="arrange_form" method="post"></form>
									...
									<td height="30">
										<table width="100%" border="0" cellspacing="0" cellpadding="0">
											<tr align="center" style="font-size: 25px;">
												...
												<td width="1101" background="img/tab_05.gif">维修安排管理</td>
												...
												<td bgcolor="#f3ffe3" height="390" valign="top">
														...
														<tr align="center" height="50">
															<td valign="top" class="mt10">请选择查询条件:<select
																name="searchName" class="select">
																	<option value="arrange.aID">维修单编号</option>
																	<option value="arrange.empID">员工编号</option>
																	<option value="employee.empName">员工姓名</option>
																	<option value="arrange.yID">预约单编号</option>
																	<option value="yuyue.cusID">客户编号</option>
																	<option value="yuyue.cusName">客户姓名</option>
																	<option value="yuyue.carnum">车牌号</option>
																	<option value="yuyue.carmoder">车辆类型</option>
																	<option value="yuyue.repairitem">修理项目</option>
													...
													<table>
														<tr>
															<td><img src="img/311.gif" width="16" height="16" />
																<span class="STYLE4">维修信息列表</span></td>
														</tr>
													</table>
													<table width="99%" border="0" align="center"
														...
														<tr>
															...
															<td width="10%" height="30" background="img/tab_14.gif"
																bgcolor="#FFFFFF" align="center">员工电话</td>
															<td width="9%" height="30" background="img/tab_14.gif"
														...
														<tbody align="center" onmouseover="changeto()"
															...
															<c:forEach var="item" items="${arranges}"
																varStatus="arrange">
																<tr align="center" bgcolor="#FFFFFF" height="25" >
																	...
																	<td>${item.cusName}</td>
																	<td>${item.cusphone}</td>
																	<td><a href="Arrange?operator=5&&aID=${item.aID}">查看</a></td>
																	<c:if test="${item.modifyflag==0}">
																		<td><a href="Arrange?operator=3&&aID=${item.aID}"><input
																			type="button" name="button"
																			class="btn btn82 btn_save2" value="修改">
																	</a></td>
																	<td><a href="Arrange?operator=4&&aID=${item.aID}"><input
														...
														<tr>
															<td colspan="9">共${totalRows}行&nbsp;
																第${currentPage}页&nbsp; 共${totalPage}页&nbsp; <c:if
																	test="${operator==0}">
																	<a
																		href="Arrange?operator=0&&queryName=${queryName}&&queryValue=${queryValue}&&currentPage=${currentPage}&&pagerMethod=1">首页</a>
																	<a
																		href="Arrange?operator=0&&queryName=${queryName}&&queryValue=${queryValue}&&currentPage=${currentPage}&&pagerMethod=2">上一页</a>
																	<a
																		href="Arrange?operator=0&&queryName=${queryName}&&queryValue=${queryValue}&&currentPage=${currentPage}&&pagerMethod=3">下一页</a>
																	<a
																		href="Arrange?operator=0&&queryName=${queryName}&&queryValue=${queryValue}&&currentPage=${currentPage}&&pagerMethod=4">尾页</a>
																</c:if> <c:if test="${operator==1}">
																	<a
																		href="Arrange?operator=1&&currentPage=${currentPage}&&pagerMethod=1">首页</a>
																	<a
																		href="Arrange?operator=1&&currentPage=${currentPage}&&pagerMethod=2">上一页</a>
																	<a
																		href="Arrange?operator=1&&currentPage=${currentPage}&&pagerMethod=3">下一页</a>
																	<a
																		href="Arrange?operator=1&&currentPage=${currentPage}&&pagerMethod=4">尾页</a>
																</c:if>
															</td>
														</tr>
													</table></td>
