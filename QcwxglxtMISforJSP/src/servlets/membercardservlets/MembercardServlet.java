 ...
 */
@WebServlet(name = "Membercard", description = "Membercard", urlPatterns = { "/Membercard" })
public class MembercardServlet extends HttpServlet {
		...
		if (StringUtils.isNotEmpty(operator)) {
			...
			if (StringUtils.equals(operator, "0") || StringUtils.equals(operator, "1")) {
				...
				try {
					...
					if(mcs.size()==0){
						out.print("<script language='javascript'>alert('未检索到符合条件的记录');window.location='membercard.jsp';</script>");
						return;
					} else {
					...
					out.print("<script language='javascript'>alert('查找会员卡失败！');window.location='membercard.jsp';</script>");
				}
				getServletConfig().getServletContext().getRequestDispatcher("/membercard.jsp").forward(request, response);
				return;
				...
				Integer cusId = membercard.getCusId();
				CustomerDAO cdao = new CustomerDAO();
				Customer cus;
				try {
					cus = cdao.findById(cusId);
					if(cus != null) {
						membercard.setCusName(cus.getCusName());
					}
				} catch (Exception e) {
					e.printStackTrace();
				}
			...
			if(operatorStr.equals("add") ||operatorStr.equals("modify")) {
				...
				try {
					if (operatorStr.equals("add")) {	
						...
						out.print("<script language='javascript'>alert('成功添加会员卡信息！');window.location='membercard.jsp';</script>");
					}
					else {
						mdao.update(membercard1);
						out.print("<script language='javascript'>alert('成功修改会员卡信息！');window.location='membercard.jsp';</script>");
					}
				}catch(Exception e)	{
					if (operatorStr.equals("add"))
		    	...
		    	try {
		    		...
		    		out.print("<script language='javascript'>alert('成功删除会员卡！');window.location='membercard.jsp';</script>");
		    	}catch(Exception e)	{
		    		out.print("<script language='javascript'>alert('删除会员卡失败！');window.location='membercard.jsp';</script>");
		    	}
			}
		}
	}
